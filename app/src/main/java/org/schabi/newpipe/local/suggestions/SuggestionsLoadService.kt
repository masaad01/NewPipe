 ...
 * SuggestionsLoadService.kt is part of NewPipe
 *
 * License: GPL-3.0+
...
import org.schabi.newpipe.local.suggestions.SuggestionsEventManager.postEvent

class SuggestionsLoadService : Service() {
    companion object {
        private val TAG = SuggestionsLoadService::class.java.simpleName
        const val NOTIFICATION_ID = 7293451
        private const val ACTION_CANCEL = App.PACKAGE_NAME + ".local.suggestions.service.SuggestionsLoadService.CANCEL"
    ...
    private lateinit var suggestionsLoadManager: SuggestionsLoadManager
    ...
    override fun onCreate() {
        super.onCreate()
        suggestionsLoadManager = SuggestionsLoadManager(this)
    }
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        loadingDisposable = suggestionsLoadManager.startLoading()
    ...
    private fun setupNotification() {
        ...
        val throttleAfterFirstEmission = Function { flow: Flowable<SuggestionsLoadState> ->
            flow.take(1).concatWith(flow.skip(1).throttleLatest(NOTIFICATION_SAMPLING_PERIOD.toLong(), TimeUnit.MILLISECONDS))
        }

        notificationDisposable = suggestionsLoadManager.notification
    ...
    private fun updateNotificationProgress(state: SuggestionsLoadState) {
